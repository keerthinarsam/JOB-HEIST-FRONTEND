...
let axios = Axios.create({baseURL: 'https://job--heist.herokuapp.com/'});
...
class Appliers extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      if(response.data.length){
        this.setState({Totalappliers:response.data.length})
        ...
        response.data.map( async (job)=> {
          ...
          // 
            ...
            this.setState({appliersResumes:[...this.state.appliersResumes,response.data]});
          //
          } 
        })
      }
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.jobId !== this.props.jobId) {
      this.setState({ status :'' , appliersResumes : [] , Totalappliers:''},()=>{this.updateAppliersResumes()})
    }
  }
  ...
  handleClick = async (appliedJobId,status,index) => {
    ...
    try {
      ...
      let copyOfAppliersResumes = this.state.appliersResumes;
  ...
  render() {
    return (
      <div className="p-5">
        {this.props.jobId ? 
        (this.state.status === 'done' && this.state.appliersResumes.length) ?
        this.state.appliersResumes.map((resume,index) => {
  return <div key={index} className="mb-5 pl-5  border border-dark shadow-lg rounded bg-white">
            ...
            <div className='text-right p-4'>
              { !resume.status ?
              <span> 
              <button type="button" className="btn btn-sm border-danger btn-outline-danger mr-4" value='rejected' onClick={(e)=>{this.handleClick(resume.appliedJobId , e.target.value , index)}}>REJECT</button>
              <button type="button" className="btn btn-success btn-sm" value='accepted' onClick={(e)=>{this.handleClick(resume.appliedJobId , e.target.value , index)}}>ACCEPT</button>
              </span>
              :
              <span className={resume.status === 'accepted' ? 'text-success font-weight-bold' : 'text-danger font-weight-bold' }> {resume.status === 'accepted' ? 'ACCEPTED SUCCESSFULLY' : 'REJECTED SUCCESSFULLY' } </span>
              }
              </div>
        ...
        <div name='noAppliersFound'>
           <div  className="card text-center  shadow-lg p-1  rounded-pill border border-danger " >
              <div className="card-body">
                <h4 className="card-title">SORRY</h4>
                <p className="card-text m-1">No Appliers Found.</p>
              </div>
          </div>
        </div>
        : null}
...
export default connect(mapStateToProps, mapDispatchToProps)(Appliers);
