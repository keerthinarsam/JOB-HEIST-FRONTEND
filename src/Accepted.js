...
class Accepted extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      if(response.data.length){
        this.setState({TotalAcceptedResumes:response.data.length})
        ...
        response.data.map( async (job)=> {
          ...
          // 
            ...
            this.setState({acceptedResumes:[...this.state.acceptedResumes,response.data]});
          //
          } 
        })
      }
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.jobId !== this.props.jobId) {
      this.setState({ status :'' , acceptedResumes : [] , TotalAcceptedResumes:''},()=>{this.updateAppliersResumes()})
    }
  }
  ...
  //       }
  //     });
  //     let copyOfAppliersResumes = this.state.acceptedResumes;
  //         copyOfAppliersResumes[index].status = status;
  //     this.setState({acceptedResumes:copyOfAppliersResumes})
  ...
  render() {
    return (
      <div className="p-5">
        {this.props.jobId ? 
        (this.state.status === 'done' && this.state.acceptedResumes.length) ?
        this.state.acceptedResumes.map((resume,index) => {
  return <div key={index} className="mb-5 pl-5  border border-dark shadow-lg rounded bg-white">
            ...
            <div className='text-right p-4'>
              <span className='text-success font-weight-bold'> ACCEPTED RESUME </span>
            </div>
